...
CREATE_ONION_DOMAINS_SRC_PATH=$(dirname "$(readlink -f "$0")")
...
function load_dependencies() {
  ...
  #another module.
  echo "CREATE_ONION_DOMAINS_PATH=$CREATE_ONION_DOMAINS_PATH"
  if [[ "$CREATE_ONION_DOMAINS_PATH" == *"/dependencies/bash-create-onion-domains" ]]; then
...
function load_required_dependency() {
  ...
  local dependency_dir="$CREATE_ONION_DOMAINS_PATH/$dependency_or_parent_path/$dependency_name"
...
function load_parent_dependencies() {
  ...
  for parent_dep in "${parent_dependencies[@]}"; do
    ...
    if [ ! -d "$CREATE_ONION_DOMAINS_PATH/../$parent_dep" ]; then
      ...
      source "$CREATE_ONION_DOMAINS_PATH/../$parent_dep/src/main.sh"
    fi
